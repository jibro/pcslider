            ...
            if (!a.loadPlus || !a.loadPlus()) {
                ...
                if (f == "bsharesync") j = [a.whost, "/bsyncShare?site=", f].join(""), a.updateCounter(), a.shareStats(f), d.submitForm(j, c, "_blank");
                    ...
                    if (h.bShareLoad)
                        ...
                        f == "gmw" ? j = d.replaceParam("${URL}", c.url.replace("http://", "https://"), j) : c.url && (j = d.replaceParam("${URL}", c.url, j)), j = d.replaceParam("${TITLE}", c.title, j), j = d.replaceParam("${CONTENT}", c.summary, j), j = d.replaceParam("${IMG}", c.pic, j), j = d.replaceParam("${VIDEO}", c.video, j);
